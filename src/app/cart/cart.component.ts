...
export class CartComponent {
  ...
  update(event: any, index?: number) {
    ...
    if(index === undefined) {
      ...
      this.task().completed = this.task().subtasks?.every(t => t.completed) ?? true
    }
  ...
  incrementQuantity(index: number) {
    this.task().subtasks![index].order.quantity +=  1;  
    this.calTotalPrice();
    this.orderService.updateOrder(this.task().subtasks![index].order.id, this.task().subtasks![index].order.quantity).subscribe(res => {
  ...
  calTotalPrice() {
    this.totalPrice = 0;
    this.task().subtasks?.filter(t => t.completed).forEach(t => {
      this.totalPrice += t.order.quantity * t.order.idProd.price;
    }) 
  }
  ...
  submitVoucher() {
    ...
    this.voucher = this.vouchers.filter(v => v.name === voucherCode)[0]?.value ?? 0;
    return this.voucher;        
  }
